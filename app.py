...
Languages=['Hindi','English','Punjabi','Marathi']
Ratings = [1,2,3,4,5]
...
@app.route('/song/<int:n>',methods=["GET","POST"])
def song(n):
    used_song = Song.query.filter_by(id=n).first()
    return render_template('song.html',usong=used_song)
...
def Give_rating():
    return render_template('/add/add_rating.html',Ratings=Ratings)
